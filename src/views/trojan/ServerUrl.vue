...
const HandleDel = (row) => {
  ...
  ElMessageBox.confirm('确定删除么?')
    .then(() => {
      delTrojanUrlServer({ id: row.id }).then((res) => {
        HandleSearch()
...
const HandleBatchDel = (rows) => {
  ...
  ElMessageBox.confirm('确定删除么?')
    .then(() => {
      ...
      delBatchTrojanUrlServer(ids).then((res) => {
        HandleSearch()
        ElMessage.success('删除成功')
      })
    })
    ...
    ElMessageBox.confirm('确定提交么?')
      .then(() => {
        if (formData.value.id) {
          //编辑
          updateTrojanUrlServer(formData.value).then((res) => {
            HandleSearch()
          ...
          //新增
          addTrojanUrlServer(formData.value).then((res) => {
            HandleSearch()
            dialogVisible.value = false
...
const HandleSearch = (page) => {
  ...
  getTrojanUrlServer(filters.value).then(res => {
...
<template>
  ...
  <el-table ref="refTable" :data="tableData" highlight-current-row @selection-change="HandleSelectChange"
    ...
    <el-table-column prop="servername" label=" 链接名称" width="250"></el-table-column>
